...
get p c d = listToMaybe <$> query "SELECT component, description, date_added, archived, array[] :: text[] AS tags FROM portfolio.project_components WHERE project = ? AND component = ? AND date_added = ?" (p, c, d)

----------------------------------------------------------------------

adminList :: HasPostgres m => Text -> m [Component]
adminList project = query "SELECT component, description, date_added, archived, array[] :: text[] AS tags FROM portfolio.project_components WHERE project = ? ORDER BY date_added" (Only project)

add :: (HasPostgres m, Functor m) => Text -> Component -> m (Either Text Int64)
add project c = toEither' $ execute "INSERT INTO portfolio.project_components (project, component, description, archived) VALUES (?, ?, ?, ?)" (project, component c, description c, archived c)

edit :: (HasPostgres m, Functor m) => Text -> Component -> m (Either Text Int64)
edit project c = toEither' $ execute "UPDATE portfolio.project_components SET description = ?, archived = ? WHERE project = ? AND component = ? AND date_added = ?" (description c, archived c, project, component c, date c)
